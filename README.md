...
- `TAVILY_API_KEY` in `.env` is optional and off by default. Without it, the AI only answers from what its model already knows, which for specific game facts (an exact item location, a boss's weak point) is often wrong or a guess, since small local models don't have that level of detail memorized. With it set, the bot runs a quick web search alongside every mention reply and gives the AI the top few results to work from, so those specific answers are much more likely to be accurate - unlike the AI chat itself, this does send your message (with the trigger word stripped out) to Tavily's search API over the internet. Get a free key (no credit card required, 1,000 searches a month free) at [app.tavily.com](https://app.tavily.com/) (the installer asks for this too and lets you skip it), then restart the bot (re-run the installer, or `pm2 restart discord-bot`) to pick it up.
