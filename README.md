...
Logs go to `bot.log` and `bot.error.log` inside the project folder, useful if a command does not seem to be working. `pm2 logs discord-bot` shows the same thing live in the terminal, and `pm2 status` shows whether it's currently running.
...
- `ITAD_COUNTRY` in `.env` sets which country's prices and currency the bot uses everywhere - watching, `!price`, `!prices`, `!info`, `!list`, `!history`, and the daily check alerts. It's a 2-letter country code (`US`, `GB`, `DE`, `CA`, `AU`, and so on); defaults to `US` if left blank. This is one setting for the whole bot, not per-person, since watched games and their alerts are shared across everyone in the channel. Change it in `.env` (or re-run the installer, which asks for this too) and restart (`pm2 restart discord-bot`) to pick it up.
...
- Not every store reports when a sale ends, so the "ends in X days" note only shows up when that data is available - no note doesn't mean the sale is permanent, just that the end date isn't known. If a sale you're already watching is within 48 hours of ending by the time of a later daily check, you get a separate one-time "ending soon" reminder in the same channel, even if the price itself hasn't changed since the original alert. To change that 48-hour window, edit `SALE_ENDING_SOON_HOURS` near the top of `index.js` and restart it.
...
- The installer manages the bot with [pm2](https://pm2.keymetrics.io), so once it's set up you can check on it with plain pm2 commands any time: `pm2 status` (is it running), `pm2 logs discord-bot` (live logs), `pm2 restart discord-bot`, `pm2 stop discord-bot`, and `pm2 delete discord-bot` (remove it from pm2 entirely - re-run the installer to add it back).
...
- The join greeting posts to whichever channel is named `general` in your server. To use a different channel, change `GREETING_CHANNEL_NAME` near the top of `index.js` to that channel's name (no `#`) and restart it (re-run the installer, or `pm2 restart discord-bot`). If no channel with that name exists, nothing posts and `bot.error.log` will note it.
- Any Discord Scheduled Event on your server gets an automatic reminder posted to that same channel 15 minutes before it starts, like "EVENT: Movie Night starting in 15 minutes!" Each event only gets reminded once. To change how far ahead it reminds, edit `EVENT_REMINDER_MINUTES` near the top of `index.js` and restart it.
...
- The bot's name, trigger word, and personality all come from `personality.yaml`. It ships set up as BMO from Adventure Time:
  ...
  ```
  Change `name` to whatever the bot should be called (used in a couple of log messages), `trigger` to the word that should make it reply (matched as a whole word, case-insensitive, anywhere in a message), and `persona` to a description of how it should talk and act - write it like you're describing a character to someone. It always stays an expert on this README and on video games in general, no matter what personality you give it - quick banter stays a short one-liner, but a real question (how to use the bot, or a game question) can run to two or three short sentences so the actual answer isn't cut off. Edit the file and restart it (re-run the installer, or `pm2 restart discord-bot`) to pick up the changes. `greetings.yaml` and `goodbyes.yaml` have their own sample lines written in BMO's voice too - edit those separately if you want the join/leave messages to match a different personality.
- To turn the AI answers off without uninstalling Ollama (for example if you want your computer's resources back for something else), set `AI_ENABLED=false` in `.env` and restart it (re-run the installer, or `pm2 restart discord-bot`). Set it back to `true` (or remove the line entirely) to turn it back on. This only affects mention replies - everything else keeps working either way.
